...
```js
import { createServer } from "node:http"; const server = createServer((request, response) => {   response.end("Hello World!"); }); server.listen(3000, () => {   console.log("Server: http://localhost:3000"); });
```
## Response
...
`const server = createServer((request, response) => {   response.statusCode = 200;   response.setHeader("Content-Type", "text/plain; charset=utf-8");   response.end("Olá Mundo."); });`
...
```js
const server = createServer((request, response) => {   response.setHeader("Content-Type", "text/plain; charset=utf-8");   if (request.method === "GET" && request.url === "/") {     response.statusCode = 200;     response.end("Home.");   } else if (request.method === "POST" && request.url === "/produto") {     response.statusCode = 201;     response.end("Produto criado.");   } else {     response.statusCode = 404;     response.end("Não encontrada.");   } });
```